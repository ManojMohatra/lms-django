...
def create_quiz(request, course_id):
    """Allows teachers to initiate a new quiz for their specific course."""
    ...
    return render(request, 'quiz/create_quiz.html', {'course': course})
...
def take_quiz(request, quiz_id):
    ...
    is_enrolled = Enrollment.objects.filter(student=request.user, course=quiz.course).exists()
    if not is_enrolled and quiz.course.teacher != request.user:
...
def manage_quiz_results(request, quiz_id):
    ...
    if request.user.profile.role != "teacher" or quiz.course.teacher != request.user:
